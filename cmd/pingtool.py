...
def parse(ping_output, domain):
    ...
    host = socket.getaddrinfo(domain, 'http')
    # print(host[0][4][0])
...
def main(argv=sys.argv):
    ...
    parser.add_option("-d", "--doman",
                      help="请求URL，必填;如含有特殊参数字符，URL使用双引号括起来")
    ...
    cmd = "ping -c " + options.count + " " + options.doman
    ping_output = os.popen(cmd)
    lines = ping_output.readlines()
    # print(lines)
    ls = ""
    for line in lines:
        l = line.strip('\n')
    ...
    ping_result = parse(ls, options.doman)
